    ...
    <div class="s-trips py-8">
      <div class="container mx-auto">
        <div class="flex justify-between items-center s-trips-top">
          <div>
            <h2 class="text-black text-2xl">Ваши сопровождающие</h2>
          </div>
        </div>
        <div class="s-trips-table pt-8">
          ...
          >
            <div>
              ...
              >
                <div class="px-3.5 w-44">Имя</div>
                <div class="px-3.5 w-44">Фамилия</div>
                <div class="pl-3.5 w-44">Марка автомобиля</div>
              </div>
            </div>
            <div
              v-for="(item, key) in attendants"
              :key="key"
              class="hover:bg-header-blue transition duration-500 ease-in-out s-trops-table-linewrap"
            >
              <a
                @click="onNavigate('/attendants/' + item.id)"
              ...
              >
                ...
                <span class="pl-3.5 w-44">{{ item.car_model }}</span>
              </a>
            </div>
          </div>
        </div>
...
import { Attendants } from "../types/attendants";

export default defineComponent({
  name: "AttendantsComponent",
  components: {
    HeaderComponent
  },
  setup() {
    const attendants = ref<Attendants[]>([]);
    const links = ref<object>({});
    const current_page = ref<number>(0);
    return { attendants, links, current_page };
  },
  mounted() {
    this.getData("/api/v1/attendants");
  },
  methods: {
        ...
        .then(function (response: any) {
          response.data.data.forEach((item: any) => {
            const itemData = {
              ...
              car_model: item.attributes.car_model
            };
            vm.attendants.push(itemData);
          });
